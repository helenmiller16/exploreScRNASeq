---
title: "QC sdy998 data"
author: "Helen Miller"
date: "4/15/2020"
output: html_document
---

# Load, prep, and QC data
This generally follows suggestions in https://osca.bioconductor.org/index.html. 
...
This assumes that the single cell data from SDY998 has been downloaded into a `data` folder. 
...
```{r immport meta, eval = FALSE, echo = FALSE}
...
NOTE: the CEL-seq protocol says that spike-ins were used, but I cannot find that data anywhere, so will not include that in QC. 

It appears that emtpy wells have already been removed, but I'm not sure what the `Empty` celltype is. 

https://osca.bioconductor.org/quality-control.html#choice-of-qc-metrics

### Derive per-cell QC metrics

Here's how we could go about deriving QC metrics. 
...
## Use provided QC metrics

QC metric per cell were provided, so we will use those. 

```{r provided qc metrics}
...
This data should be qc'd by batch, as it includes data from multiple donors, and has been sorted by cell-type. I'm not sure if we should also batch it based on plate. 
...
Not running this because I think it has already been done. 

```{r cellcalling, eval= FALSE, echo = FALSE}
...
Check to make sure that you haven't removed cells that were outliers because of actual biological differences. 
...
### Mark or filter bad cells
https://osca.bioconductor.org/quality-control.html#marking-qc

At this point, we have the option to move forward with either the filtered version of the data, or just mark cells that should be filtered. As suggested, we will save the filtered version for future analysis. 


```{r}
saveRDS(filtered, "data/sceQC.rds")
```
